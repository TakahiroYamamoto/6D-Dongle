...
namespace DongleManager
{
    public partial class FormMain : Form
    {
        ...
        public FormMain()
        {
            ...
            enumDongleCtl_Direct.Init(licMgr);
        ...
        {
            List<DongleListItem> dongleItems = enumDongleCtl_Direct.GetDongleListItem();
        ...
        private void btnCreateC2V_Click(object sender, EventArgs e)
        {
            ...
            try
            {
                ...
                {
                    string errmsg;
                    string c2v = LicenseManager.GetDongleC2V(dongleItem.dongleId, out errmsg);
                    if (c2v == null)
                    {
                        MessageBox.Show(errmsg);
                        return;
        ...
        private void btnLoadC2V_Click(object sender, EventArgs e)
        {
            ...
            List<DongleListItem> dongleItems = new List<DongleListItem>();
            {
                ...
                try
                {
                    ...
                    {
                        string c2v = LicenseManager.LoadFile(file, out errmsg);
                        if (c2v == null)
                            throw new Exception();
                        DongleListItem dongleItem = new DongleListItem();
                        dongleItem.c2vData = c2v;
                        dongleItem.dongleId = LicenseManager.GetDongleIdFromC2V(c2v, out errmsg);
        ...
        {
            List<DongleListItem> dongleItems = c2VListCtl.GetDongleListItem();
        ...
        private void btnLoadV2C_Click(object sender, EventArgs e)
        {
...

            {
                ...
                try
                {
                    foreach (string file in dlg.FileNames)
                    {
                        string v2cData = LicenseManager.LoadFile(file, out errmsg);
                        if (v2cData == null)
                            throw new Exception();
                        DongleListItem dongleItem = new DongleListItem();

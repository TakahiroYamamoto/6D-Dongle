...
namespace DongleManager
{
    public partial class ProcessBurnDlg : Form
    {
        ...
        public ProcessBurnDlg(DongleManagerLib.LicenseManager licMgr, List<DongleListItem> dongleItems, List<int> productIdxs, string folderV2C)
        {
            InitializeComponent();
        ...
        public ProcessBurnDlg(DongleManagerLib.LicenseManager licMgr, List<DongleListItem> dongleItems)
        {
            InitializeComponent();
        ...
        void ThreadWorkDirect()
        {
            ...
            foreach (DongleListItem dongleItem in dongleItems)
            {
                ...
                try
                {
                    string c2vData = DongleManagerLib.LicenseManager.GetDongleC2V(dongleItem.dongleId, out errmsg);
                    if (c2vData == null)
                        throw new Exception("");
                    ...
                    if (v2cData != "")
                    {
                        if (DongleManagerLib.LicenseManager.BurnToDongle(dongleItem.dongleId, v2cData, out errmsg) < 0)
                            throw new Exception("");
        ...
        void ThreadWorkBurnV2C()
        {
            ...
            foreach (DongleListItem dongleItem in dongleItems)
            {
                ...
                try
                {
                    ...
                    if (v2cData != "")
                    {
                        if (DongleManagerLib.LicenseManager.BurnToDongle(dongleItem.v2c_filename, v2cData, out errmsg) < 0)
                            throw new Exception();
                        Invoke(new Delegate_EventFromThread(this.EventFromThread), count, "");
                    }
